    ...
    def MotionStreaming(self, request_iterator, context):
        ...
        for ri in request_iterator:
            if not face_embeddings:
                face_embeddings = np.array(get_face_indexes(ri.userInfo.userId))
            ...
            if len(box) > 0:
                ...
                if motion.result['label'] == ri.expectedLabel and motion.result['confidence'] > 0.5:
                    encoding = face_recognition.face_encodings(image, box)
                    compare_result = face_recognition.compare_faces(face_embeddings, encoding[0])
        ...
        if len(face_embeddings) > 0:
            ...
            faces.update_one({'user_id': user_id}, {'$set': {'encodings': encodings}})
